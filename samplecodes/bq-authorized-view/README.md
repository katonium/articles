...
**共通の権限チェック**: 承認済みビュー・承認済みデータセットのどちらでも、ビューを作成・編集するユーザーは参照先テーブルへの `bigquery.tables.getData` 権限が必要です。権限のないユーザーがビューの SQL を改ざんして機密データを取得することは、どちらの方式でも防がれます。

**承認済みビュー** は、ビュー単位で個別に承認を行います。新しいビューを追加するたびに、参照先データセットの承認リストに登録が必要です。

**承認済みデータセット** は、データセット全体を一括で承認します。承認済みデータセット内に将来追加されるビューも自動的に承認されるため、管理の手間が軽減されます。多数のビューを運用する場合に適しています。
...
| 2 | 異常系 | 新規ビュー作成 | user3 | dataset_d に dataset_a 参照の新規ビューを作成 | **403 拒否**。承認済みビューと同様、作成者に参照先への権限が必要 |
| 3 | 異常系 | ビュー SQL 変更 | user3 | dataset_d 内のビュー SQL を全カラム参照に変更 | **403 拒否**。承認済みビューと同様、更新者に参照先への権限が必要 |
